...
namespace Ziusudra.Rencode
{
        ...
        Encoder<ICollection>
    {
        ...
        {
            return (header == CHR_LIST) || (header >= FIXED_START && header <= byte.MaxValue);
        }
        ...
        protected async override ValueTask<ICollection> DoReadValueAsync(IRencodeReader reader, byte header, CancellationToken cancellationToken)
        {
            ...
            {
                object? current = await reader.ReadValueAsync(cancellationToken, _TermEncoder)
                ...
                while (current is not byte b || b != CHR_TERM)
                {
                    ...
                    current = await reader.ReadValueAsync(cancellationToken, _TermEncoder)
                        .ConfigureAwait(false);
                    if (current == null)
                        throw new RencodeException(SR.RencodeException_ValueWasExpected);
                }
        ...
        protected async override ValueTask DoWriteValueAsync(IRencodeWriter writer, ICollection value, CancellationToken cancellationToken)
        {
            ...
            foreach (var item in value)
                await writer.WriteValueAsync(item, cancellationToken)
                    .ConfigureAwait(false);
        ...
        internal const byte FIXED_COUNT = 64;
